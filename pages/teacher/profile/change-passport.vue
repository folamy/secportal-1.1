<template>
    <v-layout row wrap>
      <panel title="Change Passport" class="panel ">
        <!-- <v-flex  class="" > -->
        <h3 class="center mt-5">Select Passport <i class="studentNames">{ {{teacher.lastname + ' ' + teacher.firstname}} }</i></h3>
...
export default {
  ...
  methods: {
    ...
    async changePassport (event) {
      ...
      const response = await this.$axios.put(`/t-change-passport/${teacherID}`,
        formData,
        {
          headers: {
      ...
      this.$store.commit('setTeacher', response.data)
      this.$router.push({

<template>
  <v-layout row wrap>
    <panel title="Payments > Returning Student" class="panel">
      <v-flex xs9 class="tableItems mt-2" v-if="student.returning">
        <v-data-table
         :headers="headers"
         :items="schoolItems"
         hide-actions
...
export default {
  ...
  methods: {
    async callback (response){
      ...
      let update = await this.$axios.post(`/returning-payment/${id}`, response)
  ...
  async fetch ({ store, params, redirect, app: { $axios } }) {
    ...
    for (var i = 0; i < pays.length; i++) {
      if (pays[i].class === student.classLevel && pays[i].term === term && pays[i].purpose === 'newIntake') {
